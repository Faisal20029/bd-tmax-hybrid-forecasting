...
metrics_data <- data.frame(
  Station = rep(c("Dhaka", "Chittagong", "Barisal", "Bogura", "Comilla", 
                  ...
                  "Mymensingh"), each = 9),
  Model = rep(c("SARIMA", "LSTM", "XGBoost", "ANN", "SVR", "SARIMA+LSTM", 
...
# Model order and factors
model_levels <- c("SARIMA", "LSTM", "XGBoost", "ANN", "SVR", 
                  "SARIMA+LSTM", "SARIMA+ANN", "SARIMA+XGBoost", "SARIMA+SVR")
metrics_data$Model <- factor(metrics_data$Model, levels = model_levels)

# Positions
station_levels <- unique(metrics_data$Station)
metrics_data$StationNum <- as.numeric(factor(metrics_data$Station, levels = station_levels))
metrics_data$ModelNum <- as.numeric(metrics_data$Model)
n_models <- length(model_levels)
model_offsets <- setNames((seq_len(n_models) - (n_models + 1) / 2) * (0.8 / n_models), model_levels)
metrics_data$x_pos <- metrics_data$StationNum + model_offsets[as.character(metrics_data$Model)]
...
lowest <- metrics_data %>%
  ...
  ungroup() %>%
  mutate(y_pos = MAE * 1.05)

# Model colors
model_colors <- c(
  "SARIMA" = "#1f77b4", "LSTM" = "#ff7f0e", "XGBoost" = "#2ca02c",
...
ggplot(metrics_data, aes(x = Station, y = MAE, fill = Model)) +
  ...
  geom_text(data = lowest, aes(x = x_pos, y = y_pos, label = "*"), 
            color = "black", fontface = "bold", size = 5, inherit.aes = FALSE) +
  scale_fill_manual(values = model_colors) +
  scale_x_discrete(limits = station_levels) +
  labs(
    title = "MAE Comparison Across 16 Stations (* = Best Model)",
  ...
  annotate("text", x = 1, y = max(metrics_data$MAE) * 0.95,
           label = "* indicates model with lowest MAE for each station",
...
# Save the plot
ggsave("RMSE_Comparison_No_Markers.png", width = 12, height = 6, dpi = 300)
